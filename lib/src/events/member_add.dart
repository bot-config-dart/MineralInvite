...
class MineralInviteMemberJoin extends MineralEvent<MemberJoinEvent> with InvitesMineral, Container {
  ...
  Future<void> handle(event) async {
    ...
    Invite? invite = invites.find((element) =>
        !cachedInvites.invites.containsKey(element) &&
        cachedInvites.invites.get(element.code)!.uses < element.uses);
    ...
    if (invite == null) {
      if(event.guild.features.contains(GuildFeature.vanityUrl) && event.guild.vanity!.uses > invitesCache.vanityInvite!.uses) {
        manager.controller.add(InviteMemberAdd(event, null, vanityInvite:  event.guild.vanity,
            isVanity: true));
        return;
      }
    }
    ...
    manager.controller.add(InviteMemberAdd(event, invite,
        isVanity: false));
  }
}
